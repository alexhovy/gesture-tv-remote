...
class GestureSessionTests(unittest.TestCase):
    ...
    def test_volume_return_to_center_suppresses_repeat_until_released(self) -> None:
        ...
        gesture_released = session.evaluate(
            [
                primary,
                _hand_state(GESTURE_OPEN_PALM, center=(0.70, 0.51), size=0.20),
            ],
            now=0.45,
        )
        second_start = session.evaluate(
    ...
    def test_volume_motion_allows_opposite_after_neutral_reset(self) -> None:
        ...
        strong_opposite = session.evaluate(
            [
                primary,
                _hand_state(GESTURE_PINCH, center=(0.70, 0.39), size=0.20),
            ],
            now=0.4,
        )
    ...
    def test_volume_new_direction_rearms_after_neutral_rebase(self) -> None:
        ...
        first_down = session.evaluate(
            [
                primary,
                _hand_state(GESTURE_PINCH, center=(0.70, 0.80), size=0.20),
            ],
            now=0.1,
        )
        returning = session.evaluate(
            [
                primary,
                _hand_state(GESTURE_PINCH, center=(0.70, 0.62), size=0.20),
            ],
            now=0.2,
        )
        neutral = session.evaluate(
    ...
    def test_volume_return_stroke_does_not_emit_opposite_direction(self) -> None:
        ...
        returning = session.evaluate(
            [
                primary,
                _hand_state(GESTURE_PINCH, center=(0.70, 0.68), size=0.20),
            ],
            now=0.2,
        )
        rearmed = session.evaluate(
        ...
        crossed_return = session.evaluate(
            [
                primary,
                _hand_state(GESTURE_PINCH, center=(0.70, 0.42), size=0.20),
            ],
            now=0.5,
        )
        settled = session.evaluate(
        ...
        second_up = session.evaluate(
            [
                primary,
                _hand_state(GESTURE_PINCH, center=(0.70, 0.31), size=0.20),
            ],
            now=0.8,
        )
    ...
    def test_volume_opposite_direction_works_after_gesture_release(self) -> None:
        ...
        session.evaluate(
            [
                primary,
                _hand_state(GESTURE_OPEN_PALM, center=(0.70, 0.50), size=0.20),
            ],
            now=0.2,
        )
        second_start = session.evaluate(
    ...
    def test_volume_uses_fresh_start_after_secondary_hand_loss(self) -> None:
        ...
        second_up = session.evaluate(
            [
                primary,
                _hand_state(GESTURE_PINCH, center=(0.70, 0.74), size=0.20),
            ],
            now=0.4,
        )
