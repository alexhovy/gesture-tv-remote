...
class SessionVolumeTests(unittest.TestCase):
    ...
    def test_volume_uses_fixed_neutral_band_from_initial_pinch(self) -> None:
        ...
        self.assertIsNotNone(inside.volume_debug)
        assert inside.volume_debug is not None
        self.assertEqual(inside.volume_debug.anchor, (0.70, 0.50))
        self.assertEqual(inside.volume_debug.anchor_y, 0.50)
        self.assertEqual(inside.volume_debug.current, (0.70, 0.64))
        self.assertAlmostEqual(inside.volume_debug.activation_distance, 0.184)
        self.assertAlmostEqual(inside.volume_debug.neutral_distance, 0.160)
        self.assertTrue(inside.volume_debug.in_neutral)
