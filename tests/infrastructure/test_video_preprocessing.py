...
class VideoPreprocessingTests(unittest.TestCase):
    ...
    def test_detection_crop_lags_display_crop(self) -> None:
        controller = CameraZoomController(
            app_config(
                auto_zoom_enabled=True,
                auto_zoom_min=1.0,
        ...
        self.assertLess(
            controller.current_crop().width, controller.detection_crop().width
        )
        self.assertLess(
            controller.current_crop().height, controller.detection_crop().height
        )
    ...
    def test_auto_zoom_display_follows_hand_when_detection_hand_reaches_crop_edge(
        self,
    ) -> None:
        controller = CameraZoomController(
            app_config(
        ...
        display_crop = controller.current_crop()
        detection_crop = controller.detection_crop()
        self.assertTrue(changed)
        self.assertNotEqual(display_crop, CropRect(0.0, 0.0, 1.0, 1.0))
        self.assertGreater(detection_crop.width, display_crop.width)
        self.assertGreater(detection_crop.height, display_crop.height)

    def test_auto_zoom_display_follows_original_space_hand_near_current_crop_edge(
        self,
    ) -> None:
        controller = CameraZoomController(
            app_config(
        ...
        changed = controller.update(
            [[_landmark(0.48, 0.70), _landmark(0.52, 0.74)]],
            CropRect(0.0, 0.0, 1.0, 1.0),
        )
    ...
    def test_detection_crop_stays_wider_when_display_tracks_edge_hand(self) -> None:
        controller = CameraZoomController(
            app_config(
                auto_zoom_enabled=True,
                auto_zoom_min=1.0,
        ...
        controller.update(
            [[_landmark(0.25, 0.25), _landmark(0.75, 0.75)]],
            CropRect(0.0, 0.0, 1.0, 1.0),
        )

        changed = controller.update(
            [[_landmark(0.48, 0.65), _landmark(0.52, 0.68)]],
            CropRect(0.0, 0.0, 1.0, 1.0),
        )
    ...
    def test_auto_zoom_conditionally_updates_when_frozen_hands_near_current_crop_edge(
        self,
    ) -> None:
        controller = CameraZoomController(
            app_config(
    ...
    def test_auto_zoom_conditionally_holds_when_frozen_hands_are_inside_current_crop(
        self,
    ) -> None:
        controller = CameraZoomController(
            app_config(
