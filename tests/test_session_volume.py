...
class SessionVolumeTests(unittest.TestCase):
    ...
    def test_volume_movement_accumulates_before_threshold(self) -> None:
        ...
        session.evaluate(
            [
                primary,
                hand_state(GESTURE_PINCH, center=(0.70, 0.50), size=0.20),
            ],
            now=0.0,
        )
        first_under_threshold = session.evaluate(
            [
                primary,
                hand_state(GESTURE_PINCH, center=(0.70, 0.52), size=0.20),
            ],
            now=0.1,
        )
        second_under_threshold = session.evaluate(
            [
                primary,
                hand_state(GESTURE_PINCH, center=(0.70, 0.54), size=0.20),
            ],
            now=0.2,
        )
        crossed_threshold = session.evaluate(
            [
                primary,
                hand_state(GESTURE_PINCH, center=(0.70, 0.56), size=0.20),
            ],
            now=0.3,
        )
    ...
    def test_volume_return_to_center_suppresses_repeat_until_released(self) -> None:
        ...
        held_down = session.evaluate(
            [
                primary,
                hand_state(GESTURE_PINCH, center=(0.70, 0.60), size=0.20),
            ],
            now=0.2,
        )
        slight_return = session.evaluate(
            [
                primary,
                hand_state(GESTURE_PINCH, center=(0.70, 0.57), size=0.20),
            ],
            now=0.3,
        )
        ...
        released = session.evaluate(
            [
                primary,
                hand_state(GESTURE_PINCH, center=(0.70, 0.51), size=0.20),
            ],
            now=0.4,
        )
        gesture_released = session.evaluate(
            [
                primary,
                hand_state(GESTURE_OPEN_PALM, center=(0.70, 0.51), size=0.20),
            ],
            now=0.45,
        )
    ...
    def test_volume_return_past_center_does_not_emit_opposite_direction(self) -> None:
        session = GestureSession(AppConfig())
        primary = hand_state(GESTURE_OPEN_PALM, center=(0.20, 0.50), size=0.20)
        ...
        crossed_center = session.evaluate(
            [
                primary,
                hand_state(GESTURE_PINCH, center=(0.70, 0.48), size=0.20),
            ],
            now=0.3,
        )
    ...
    def test_volume_opposite_direction_rearms_after_held_neutral(self) -> None:
        session = GestureSession(AppConfig())
        primary = hand_state(GESTURE_OPEN_PALM, center=(0.20, 0.50), size=0.20)
        ...
        second_up = session.evaluate(
            [
                primary,
                hand_state(GESTURE_PINCH, center=(0.70, 0.44), size=0.20),
            ],
            now=1.0,
        )
    ...
    def test_volume_motion_allows_opposite_after_neutral_reset(self) -> None:
        session = GestureSession(AppConfig())
        primary = hand_state(GESTURE_OPEN_PALM, center=(0.20, 0.50), size=0.20)
        ...
        strong_opposite = session.evaluate(
            [
                primary,
                hand_state(GESTURE_PINCH, center=(0.70, 0.39), size=0.20),
            ],
            now=0.4,
        )
        ...
        self.assertEqual(mild_opposite.command_gesture, GESTURE_VOLUME_UP)
        self.assertIsNone(strong_opposite.command_gesture)

    def test_volume_new_direction_rearms_after_neutral_rebase(self) -> None:
        session = GestureSession(AppConfig())
        primary = hand_state(GESTURE_OPEN_PALM, center=(0.20, 0.50), size=0.20)
        ...
        first_down = session.evaluate(
            [
                primary,
                hand_state(GESTURE_PINCH, center=(0.70, 0.80), size=0.20),
            ],
            now=0.1,
        )
        returning = session.evaluate(
            [
                primary,
                hand_state(GESTURE_PINCH, center=(0.70, 0.62), size=0.20),
            ],
            now=0.2,
        )
        neutral = session.evaluate(
    ...
    def test_volume_return_stroke_does_not_emit_opposite_direction(self) -> None:
        session = GestureSession(AppConfig())
        primary = hand_state(GESTURE_OPEN_PALM, center=(0.20, 0.50), size=0.20)
        ...
        returning = session.evaluate(
            [
                primary,
                hand_state(GESTURE_PINCH, center=(0.70, 0.68), size=0.20),
            ],
            now=0.2,
        )
        ...
        crossed_return = session.evaluate(
            [
                primary,
                hand_state(GESTURE_PINCH, center=(0.70, 0.42), size=0.20),
            ],
            now=0.5,
        )
        ...
        mild_opposite = session.evaluate(
            [
                primary,
                hand_state(GESTURE_PINCH, center=(0.70, 0.36), size=0.20),
            ],
            now=0.7,
        )
        second_up = session.evaluate(
            [
                primary,
                hand_state(GESTURE_PINCH, center=(0.70, 0.31), size=0.20),
            ],
            now=0.8,
        )
        ...
        self.assertEqual(crossed_return.command_gesture, GESTURE_VOLUME_UP)
    ...
    def test_volume_opposite_direction_works_after_gesture_release(self) -> None:
        ...
        session.evaluate(
            [
                primary,
                hand_state(GESTURE_OPEN_PALM, center=(0.70, 0.50), size=0.20),
            ],
            now=0.2,
        )
        second_start = session.evaluate(
    ...
    def test_volume_uses_fresh_start_after_secondary_hand_loss(self) -> None:
        session = GestureSession(AppConfig())
        primary = hand_state(GESTURE_OPEN_PALM, center=(0.20, 0.50), size=0.20)
        ...
        second_up = session.evaluate(
            [
                primary,
                hand_state(GESTURE_PINCH, center=(0.70, 0.74), size=0.20),
            ],
            now=0.4,
        )
